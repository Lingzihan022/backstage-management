<template>
  <div>
    ...
    <el-card>
      ...
      <!-- tab页签区域 -->
      <!-- 定义一个计算属性判断数组长度是不是3 返回布尔值决定是否禁用按钮 -->
...
export default {
  ...
  methods: {
    ...
    async getParamsData() {
      ...
      // 选中的是3级分类
      console.log(this.selectedCateKeys)
      //   根据当前所选分类的id和当前所处的面板,获取对应的参数,发起数据请求
    ...
    editParams() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        if (res.meta.status !== 200) {
          return this.$messgae.error('修改参数失败！')
        }
        // 关闭修改参数的对话框
